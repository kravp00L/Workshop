...
foreach ($user_record in $user_list) {
    ...
    Write-LogMessage -message "Looking up account information for $($user_email)"
    try {
        $user_details = Get-MgUser -UserId $user_email
    }
    catch {
        Write-LogMessage -message "Error trying to retrieve account info for $($user_email)"
